...
class DQN(nn.Module):
  def __init__(self, img_height, img_width, num_actions):
    ...
    self.fc1 = nn.Linear(in_features = 30276, out_features = 256)
...
class EnvManager():
  ...
  def get_state(self):
    ...
    if self.just_starting():
      ...
      self.current_screen = self.get_terminal_black()   

      return self.last_state


    s1 = self.get_processed_screen()
    s2 = self.last_state
    state = s1-s2
    self.last_state = s1
    return state
...
#   rew, info = em.take_action(torch.tensor([randint(0,1)]))
